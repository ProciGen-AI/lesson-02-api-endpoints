...
**What to build.** Start from `code/06-prompt-template.py` and remove the interactive `choose()` calls. Instead, put the `FOCUSES` / `FORMATS` / `LENGTHS` options *into the prompt* and ask the model to pick the best one of each for **this** transcript, justify each pick from the call, then produce the deliverable with its own choices. Turn on **extended thinking** so the decision is inspectable; print the model's reasoning, its picks, and the final output. Run it on `call1.txt` — did its autonomous choices match what you'd have picked?
